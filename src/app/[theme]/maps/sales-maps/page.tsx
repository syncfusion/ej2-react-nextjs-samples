...
import data from './sales-map.json';
        ...
        <div className='control-pane'>
            ...
            <div className='control-section'>
                <MapsComponent id="maps" load={load} useGroupingSeparator={true} tooltipDisplayMode={"Click"} format={"n"} zoomSettings={{ enable: true, mouseWheelZoom: false, pinchZooming: false }} titleSettings={{ text: 'Sales details of products in various countries', textStyle: { size: '16px' } }}>
                    ...
                    <LayersDirective>
                        <LayerDirective shapeData={worldMap} shapePropertyPath='name' shapeDataPath='Country' dataSource={datasource.salesmap} shapeSettings={{ fill: '#C3E6ED' }} markerClusterSettings={{ allowClustering: true, allowClusterExpand: true, shape: 'Image', height: 40, width: 40, labelStyle: { color: 'white' }, imageUrl: '/nextjs/demos/images/maps/cluster.svg' }}>
                            <MarkersDirective>
                               <MarkerDirective visible={true} animationDuration={0} height={15} width={15} shape='Image' imageUrl='/nextjs/demos/images/maps/ballon.png' dataSource={datasource.salesmap} tooltipSettings={{ format: '<b>Name</b> : ${name}<br><b>Product</b> : ${product}<br><b>Total value</b> : ${worth}', visible: true, valuePath: 'name' }} />
                            </MarkersDirective>
                        </LayerDirective>
                    </LayersDirective>
                </MapsComponent>
