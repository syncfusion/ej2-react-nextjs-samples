...
export function getData(dataCount?: number): object {
    ...
    if (typeof dataCount === 'string') {
        dataCount = parseInt(dataCount) as number;
    }
    for (let i: number = 1; i <= (dataCount ?? 0); i++) {
        let code = 10000;
        tradeData.push({
        ...
        let emp = (tradeData[i - 1] as any)["Employees"];
        let sName = emp.substr(0, emp.indexOf(' ')).toLowerCase();
        (tradeData[i - 1] as any)['Mail'] = sName + (Math.floor(Math.random() * 100) + 10) + '@' + Mail[Math.floor(Math.random() * Mail.length)];
...
templateDetails.forEach(function(e: { [key: string]: any }) {
