...
import { ButtonComponent, CheckBoxComponent, ChangeEventArgs as SwitchEventArgs } from '@syncfusion/ej2-react-buttons';
...
const Overview = () => {
  ...
  const onToolbarItemClicked = (args: ClickEventArgs): void => {
    switch (args.item.text) {
      case 'Day':
        scheduleObj.current.currentView = isTimelineView.current ? 'TimelineDay' : 'Day';
        break;
      case 'Week':
        scheduleObj.current.currentView = isTimelineView.current ? 'TimelineWeek' : 'Week';
        break;
      case 'WorkWeek':
        scheduleObj.current.currentView = isTimelineView.current ? 'TimelineWorkWeek' : 'WorkWeek';
        break;
      case 'Month':
        scheduleObj.current.currentView = isTimelineView.current ? 'TimelineMonth' : 'Month';
        break;
      case 'Year':
        scheduleObj.current.currentView = isTimelineView.current ? 'TimelineYear' : 'Year';
        break;
      case 'Agenda':
    ...
    return (
      <div className = 'template'>
        <div className='icon-child'>
          <CheckBoxComponent id='timeline_views' checked={isTimelineView.current} change={onChange}/>
        </div>
          ...
          <div className='schedule-overview'>
            <AppBarComponent colorMode="Primary">
              ...
              <div className='control-panel import-button'>
                <UploaderComponent id='fileUpload' type='file' allowedExtensions='.ics' cssClass='calendar-import' buttons={{ browse: typeof document !== 'undefined' ? importTemplateFn({ text: 'Import' })[0] as HTMLElement : '' }} multiple={false} showFileList={false} selected={(onImportClick)} created={createUpload} />
              </div>
              <div className='control-panel calendar-export'>
            ...
            <div className='overview-content'>
              <div className='left-panel'>
                <div className='overview-scheduler'>
                  <ScheduleComponent id='scheduler' cssClass='schedule-overview' ref={scheduleObj} width='100%' height='100%' group={{ resources: ['Calendars'] }} timezone='UTC' eventSettings={{ dataSource: generateEvents() }} dateHeaderTemplate={dateHeaderTemplate}>
                    <ResourcesDirective>
